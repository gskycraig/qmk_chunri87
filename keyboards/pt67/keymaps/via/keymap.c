...
enum user_keys {
    OS_SW = RL_HUD,//EX_USER,
    OS_CLR,
    CH_SW,
};

#define RGB_HSV_MAX 8
static uint8_t rgb_hsvs[8][3] = {
    {HSV_RED},
    {HSV_ORANGE},
    {HSV_YELLOW},
    {HSV_GREEN},
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [WIN_FN] = LAYOUT( /* Base */
        OS_CLR,  KC_F1,   KC_F2,   KC_F3,   KC_F4,   KC_F5,    KC_F6,   KC_F7,   KC_F8,   KC_F9,   KC_F10,  KC_F11,  KC_F12,  IM_BATQ, _______, 
        OS_SW,   IM_BT1,  IM_BT2,  IM_BT3,  IM_2G4,  IM_USB,   _______, _______, _______, _______, _______, _______, _______, _______, _______,
        _______, RGB_TOG, RGB_HUI, RGB_HUD, RGB_MOD, RGB_RMOD, RGB_VAI, RGB_VAD, RGB_SPI, RGB_SPD, _______, _______, KC_MSEL,          _______, 
        _______, RL_TOG,  RL_HUI,  RL_MOD,  RL_VAI,  RL_VAD,   _______, _______, _______, _______, _______,          KC_MUTE, KC_VOLU,         
        _______, GU_TOGG, _______,                   _______,                    _______, _______, _______,          KC_MPRV, KC_VOLD, KC_MNXT
        ),
    ...
    [MAC_FN] = LAYOUT(  /* FN */
        OS_CLR,  KC_BRID, KC_BRIU, KC_MCTL,LGUI(KC_R), _______, _______, KC_MPRV, KC_MPLY, KC_MNXT, KC_MUTE, KC_VOLD, KC_VOLU, IM_BATQ, _______, 
        OS_SW,   IM_BT1,  IM_BT2,  IM_BT3,  IM_2G4,  IM_USB,   _______, _______, _______, _______, _______, _______, _______, _______, _______,
        _______, RGB_TOG, RGB_HUI, RGB_HUD, RGB_MOD, RGB_RMOD, RGB_VAI, RGB_VAD, RGB_SPI, RGB_SPD, _______, _______, _______,          _______, 
        _______, RL_TOG,  RL_HUI,  RL_MOD,  RL_VAI,  RL_VAD,   _______, _______,  _______, _______, _______,          KC_MUTE, KC_VOLU,         
...
blink_rgb_t blink_rgbs[RGB_MATRIX_BLINK_COUNT] = {
    {.index = IM_MM_RGB_BLINK_INDEX_BT1, .interval = 250, .times = 6, .color = {.r = 0x00, .g = 0xFF, .b = 0x00}, .blink_cb = im_mm_rgb_blink_cb},
    {.index = IM_MM_RGB_BLINK_INDEX_BT2, .interval = 250, .times = 6, .color = {.r = 0x00, .g = 0xFF, .b = 0x00}, .blink_cb = im_mm_rgb_blink_cb},
    {.index = IM_MM_RGB_BLINK_INDEX_BT3, .interval = 250, .times = 6, .color = {.r = 0x00, .g = 0xFF, .b = 0x00}, .blink_cb = im_mm_rgb_blink_cb},
    {.index = IM_MM_RGB_BLINK_INDEX_2G4, .interval = 250, .times = 6, .color = {.r = 0x00, .g = 0xFF, .b = 0x00}, .blink_cb = im_mm_rgb_blink_cb},
    ...
    {.index = 29, .interval = 250, .times = 3, .color = {.r = 100, .g = 100, .b = 100}, .blink_cb = NULL},
};
#endif
...
typedef union {
    ...
    struct {
        ...
        uint8_t os_mode: 2;
        uint8_t hui_index: 3;
        uint8_t rgb_togg_en: 1;
        uint8_t rl_togg_en: 1;
...
void eeconfig_confinfo_default(void) {
    ...
    confinfo.rgb_togg_en = false;
    confinfo.rl_togg_en = false;
...
bool im_process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        ...
        case RGB_TOG:
            if (record->event.pressed) {
                confinfo.rgb_togg_en = !confinfo.rgb_togg_en;
                eeconfig_update_user(confinfo.raw);
            }
            break;       
        case RL_MOD:
        ...
        case RL_TOG:
            if (record->event.pressed) {
                rgblight_toggle_noeeprom();
                confinfo.rl_togg_en = !confinfo.rl_togg_en;
                eeconfig_update_user(confinfo.raw);
            }
            return false;             
            break;  
        ...
        case RL_HUI:
            if (record->event.pressed) {
                confinfo.hui_index = (confinfo.hui_index + 1) % RGB_HSV_MAX;
                rgblight_sethsv(rgb_hsvs[confinfo.hui_index][0],
                                rgb_hsvs[confinfo.hui_index][1],
                                rgblight_get_val());
                eeconfig_update_user(confinfo.raw);
            }
        ...
        case OS_SW: {
            if (pressed) {
                if (confinfo.os_mode == WIN_BASE) {
                    ...
                    rgb_matrix_blink_set_color(29, RGB_MATRIX_MAXIMUM_BRIGHTNESS, RGB_MATRIX_MAXIMUM_BRIGHTNESS, RGB_MATRIX_MAXIMUM_BRIGHTNESS);
                    rgb_matrix_blink_set_interval_times(29, 250, 3);
                    rgb_matrix_blink_set(29);
                } else {
...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    ...
    {
        rgb_matrix_set_color(58, RGB_MATRIX_MAXIMUM_BRIGHTNESS, RGB_MATRIX_MAXIMUM_BRIGHTNESS, RGB_MATRIX_MAXIMUM_BRIGHTNESS);
    }

    if(usb_led_flag == 1 )
    {
        rgb_matrix_set_color(24, RGB_MATRIX_MAXIMUM_BRIGHTNESS, RGB_MATRIX_MAXIMUM_BRIGHTNESS, RGB_MATRIX_MAXIMUM_BRIGHTNESS);
    }
